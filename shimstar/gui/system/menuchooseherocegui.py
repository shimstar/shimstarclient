...
class MenuChooseHeroCegui(DirectObject):
	def __init__(self):
		...
		self.CEGUI.WindowManager.getWindow("ConnexionBg/Previous").subscribeEvent(PyCEGUI.PushButton.EventClicked, self, 'onPrevious')
		self.CEGUI.WindowManager.getWindow("ConnexionBg/Next").subscribeEvent(PyCEGUI.PushButton.EventClicked, self, 'onNext')
		self.CEGUI.WindowManager.getWindow("ConnexionBg/Create").subscribeEvent(PyCEGUI.PushButton.EventClicked, self, 'onCreateNewHero')
		...
		self.CEGUI.WindowManager.getWindow("ConnexionBg/InfoHero/Cancel").subscribeEvent(PyCEGUI.PushButton.EventClicked,self,'closeClicked')
		self.CEGUI.WindowManager.getWindow("ConnexionBg/InfoHero").subscribeEvent(PyCEGUI.FrameWindow.EventCloseClicked,self,'closeClicked')
		self.CEGUI.WindowManager.getWindow("ConnexionBg/InfoHero/Play").subscribeEvent(PyCEGUI.PushButton.EventClicked, self, 'onPlay')
		self.CEGUI.WindowManager.getWindow("ConnexionBg/InfoHero/Delete").subscribeEvent(PyCEGUI.PushButton.EventClicked, self, 'onDelete')
		self.CEGUI.WindowManager.getWindow("ConnexionBg/InfoHero/Confirm/Yes").subscribeEvent(PyCEGUI.PushButton.EventClicked, self, 'onDeleteConfirmed')
		self.CEGUI.WindowManager.getWindow("ConnexionBg/InfoHero/Confirm/No").subscribeEvent(PyCEGUI.PushButton.EventClicked, self, 'onCancelDelete')
		...
		self.OutDeleteAnimationInstance.setTargetWindow(self.CEGUI.WindowManager.getWindow("ConnexionBg/InfoHero/Confirm"))
		self.InDeleteAnimationInstance.setTargetWindow(self.CEGUI.WindowManager.getWindow("ConnexionBg/InfoHero/Confirm"))
	...
	def loadCharacters(self):
		...
		for ch in listOfChar:
			button=self.CEGUI.WindowManager.createWindow("Shimstar/ImageButton","ConnexionBg/group/btnPersohero=" + str(ch.getId()))
			if listOfImageSet.has_key("TempImageset" + str(ch.getFace()) )==False:
				customImageset = self.CEGUI.ImageSetManager.createFromImageFile("TempImageset" + str(ch.getFace()) , "/faces/" + str(ch.getFace()) + ".png", "images")
			...
			label=self.CEGUI.WindowManager.createWindow("Shimstar/Button","ConnexionBg/group/labelPersohero=" + str(ch.getName()))
			label.setProperty("UnifiedAreaRect", "{{" + str(0.10+0.15*i) + ",0},{0.80,0},{" + str(0.21+0.15*i) + ",0},{0.96,0}}");
	...
	def onChooseHero(self,arg):
		...
		self.CEGUI.WindowManager.getWindow("ConnexionBg/InfoHero/Delete").setUserString("id",arg.window.getUserString("id"))
		img="faces/" + face + ".png"
		self.loadImage(img)
