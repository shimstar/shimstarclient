...
class Explosion:
	...
	def preload():
		...
		for i in range(16):
			loader.loadTexture((shimConfig.getInstance().getConvRessourceDirectory() +'models/explo2/exp'+"%0"+str(2)+"d."+'png') % i)
	
	def __init__(self,render,pos,scale):
		self.expPlane = loader.loadModel(shimConfig.getInstance().getConvRessourceDirectory() +'models/plane')  #load the object
		...
		self.expTexs = self.loadTextureMovie(16, shimConfig.getInstance().getConvRessourceDirectory() +'/models/explo2/exp','png', padding = 2)
